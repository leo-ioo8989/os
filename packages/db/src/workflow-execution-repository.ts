...
async function nextJob(tx: Tx, organizationId: string, workflowId: string, objectiveId: string | null, taskId: string) {
  ...
  return tx.job.upsert({
    ...
    create: { organization: { connect: { id: organizationId } }, objective: objectiveId ? { connect: { id: objectiveId } } : undefined, task: { connect: { id: taskId } }, workflow: { connect: { id: workflowId } }, idempotencyKey, status: 'QUEUED', maxAttempts: 3, resumableState: {}, metadata: {} },
    update: {},
  });
}
...
export async function advanceWorkflowAfterJobSuccess(tx: Tx, organizationId: string, workflowId: string, jobId: string, input: AuditBase) {
  ...
  const active = await tx.job.count({ where: { organizationId, workflowId, id: { not: jobId }, status: { in: ['QUEUED','CLAIMED','RUNNING','WAITING_APPROVAL','RETRY_QUEUED'] } } });
  if (active > 0) return { kind: 'waiting_other_jobs' as const, taskId: selected.id };
  ...
  return db.$transaction(async (tx) => {
    ...
    if (workflow.currentJobId) {
      ...
      if (current.status === 'SUCCEEDED') {
        ...
        if (['advanced','completed','blocked','failed'].includes(result.kind)) await writeAudit(tx, input, AUDIT_EVENTS.WORKFLOW_RECONCILED, 'Workflow', workflowId, 'reconciled', { source: 'job_succeeded', jobId: current.id });
        return result;
      }
