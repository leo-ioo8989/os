...
describe('Phase 5', () => {
  ...
  it('bounds decomposition', () => expect(decomposeGoal({ organizationId: 'o', objectiveId: 'g', description: 'ship product', maxDepth: 1, maxTasks: 2 })).toHaveLength(2));
  it('forces optimization approval', () => expect(proposeOptimization({ targetId: 'x', changes: { a: 1 }, expectedImpact: 1, requiresApproval: false }).requiresApproval).toBe(true));
  it('denies unregistered capability', () => { const r = new CapabilityRegistry(); assert.throws(() => r.resolve('missing', 'x')); });
  it('never starts disabled workflows', () => expect(shouldStart({ workflowId: 'w', organizationId: 'o', trigger: 't', steps: ['x'], maxConcurrent: 1, requiresApproval: false, enabled: false }, true)).toBe(false));
  it('requires evidence for certification', () => {
    ...
    expect(certifyPhase5(e).status).toBe('CERTIFIED');
  });
});
