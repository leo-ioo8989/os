...
test('job lifecycle accepts only deterministic transitions', () => {
  ...
  assert.equal(canTransitionJob('FAILED','RETRY_QUEUED'), false);
});

test('retry policy terminates after max attempts', () => {
  assert.equal(retryDecision(1,3,true).status, 'RETRY_QUEUED');
  assert.equal(retryDecision(3,3,true).status, 'FAILED');
  assert.equal(retryDecision(1,3,false).status, 'FAILED');
});

test('terminal job states cannot resume', () => {
