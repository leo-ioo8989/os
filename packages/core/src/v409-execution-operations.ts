...
export type OperationalJobStatus='QUEUED'|'LEASED'|'RUNNING'|'SUCCEEDED'|'FAILED'|'CANCELLED'|'DEAD_LETTER';
export interface Job { id:string;provenance:Provenance;status:OperationalJobStatus;attempts:number;maxAttempts:number;leaseUntil?:string;workerId?:string; }
