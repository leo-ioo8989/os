...
export function appendAudit(r:AuditRecord):AuditRecord { if(!r.id||!r.timestamp||!r.actorId||!r.action||!r.decision||!r.reason)throw new Error('V406_AUDIT_FIELDS_REQUIRED');validateProvenance(r.provenance);validateAuthority(r.authority);return structuredClone(r); }
