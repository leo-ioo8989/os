...
/** Validate immutable provenance before any planning decision is produced. */
export function validatePlanningProvenance(state: PlanningState, candidate: PlanningCandidate): void {
  if (
    ...
    candidate.objectiveId !== state.objectiveId ||
    candidate.taskId !== state.taskId ||
    candidate.correlationId !== state.correlationId
  ) {
...
/**
 * Deterministically selects one next step. This function proposes only; it never
 * executes, authorizes, mutates task state, or creates another control plane.
 */
export function planNextStep(state: PlanningState, candidates: PlanningCandidate[]): AutonomousPlanProposal {
  if (!state.organizationId || !state.objectiveId || !state.taskId || !state.correlationId) {
  ...
  for (const candidate of candidates) {
    ...
    try {
      validatePlanningProvenance(state, candidate);
    } catch {
      return fail('PROVENANCE_MISMATCH');
    }
  ...
  const candidateIds = new Set(candidates.map((candidate) => candidate.stepId));
  const eligible = candidates.filter((candidate) => {
    if (completed.has(candidate.stepId)) return false;
    return candidate.dependencies.every((dependencyId) => completed.has(dependencyId) || !candidateIds.has(dependencyId));
  });
  ...
  return {
    ...
    policyVersion: V311_POLICY_VERSION,
    decision: 'PLAN',
    reason: 'NEXT_STEP_SELECTED',
    selectedStepId: selected.stepId,
    orderedEligibleStepIds: orderedIds,
  };
}
