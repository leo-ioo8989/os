...
export interface GatewayRequest extends Omit<ExecutionIntent,'risk'> { capability:string; risk:RiskLevel; approvalId?:string; expiresAt?:Date; reason?:string; }
export async function authorizeExecution(db:PrismaClient,request:GatewayRequest){
 const workers=new WorkerRepository(db); const worker=await workers.get(request.organizationId,request.workerId); if(!worker)throw new ApiError(401,'UNAUTHENTICATED','Worker authentication failed.');
 ...
 const decision=decideExecution(request.risk,hasWorkerCapability(worker,{capability:request.capability}.capability),approvalValid);
 ...
 if(decision==='REQUIRES_APPROVAL'){const approval=await new ApprovalRepository(db).create({organizationId:request.organizationId,requesterAgentId:request.workerId,targetResourceType:'Execution',targetResourceId:request.target,action:request.action,actionFingerprint:fingerprint,policyVersion:POLICY_VERSION,riskLevel:request.risk,reason:request.reason??'Privileged execution requires approval.',evidence:{capability:request.capability,target:request.target,risk:request.risk},expiresAt:request.expiresAt},{organizationId:request.organizationId,actorType:'AGENT',action:'approval_required',result:'SUCCESS'});return{decision,approvalId:approval.id,fingerprint};}
 return{decision,fingerprint};
}
export async function consumeExecutionApproval(db:PrismaClient,organizationId:string,workerId:string,approvalId:string,intent:GatewayRequest){const fingerprint=executionFingerprint(intent);const result=await new ApprovalRepository(db).consume(organizationId,approvalId,fingerprint,workerId,{organizationId,actorType:'AGENT',action:'consume',result:'SUCCESS'});if(result.kind==='missing')throw new ApiError(404,'NOT_FOUND','Approval not found.');if(result.kind!=='consumed')throw new ApiError(409,'CONFLICT','Approval is invalid, expired, already consumed, or does not match the requested action.');return{decision:'ALLOW' as const,fingerprint};}
