...
export async function authorizeExecution(db:PrismaClient,request:GatewayRequest){
 const worker=await new WorkerRepository(db).authenticate(request.organizationId,request.workerId,request.credential,{organizationId:request.organizationId,actorType:'SYSTEM',action:'authenticate',result:'SUCCESS'});
