...
function role(organizationId: string): WorkforceRole { return { roleId: 'engineer', organizationId, name: 'Engineer', category: 'engineering', description: 'bounded engineer', responsibilities: ['implement'], status: 'ACTIVE', employmentType: 'PERMANENT', capabilityRequirements: ['SOFTWARE_ENGINEERING'], contextScope: scope, qaResponsibility: { required: true, independent: true, qualityCriteria: ['output is valid'] }, escalationPolicy: { reasons: ['QUALITY_FAILURE'], target: 'LEO' }, createdAt: '2026-01-01T00:00:00.000Z', updatedAt: '2026-01-01T00:00:00.000Z' }; }
function capability(): CapabilityDefinition { return { capabilityId: 'SOFTWARE_ENGINEERING', description: 'software engineering', contract: { capabilityId: 'SOFTWARE_ENGINEERING', requiredInputs: ['task'], expectedOutputs: ['result'], qualityCriteria: ['output is valid'], risk: 'LOW', requiredContextScopes: ['PROJECT'], qa: { required: true, independent: true, qualityCriteria: ['output is valid'] }, escalation: { reasons: ['QUALITY_FAILURE'], target: 'LEO' }, modelRequirements: ['coding'], compatibleProviderCharacteristics: ['deterministic'] } }; }
function provider(): ProviderDefinition { return { providerId: 'provider-test', providerName: 'Deterministic Provider', providerType: 'TEST', availability: 'AVAILABLE', costClass: 'FREE', supportedCapabilities: ['SOFTWARE_ENGINEERING'], routingEligible: true, provenance: 'test' }; }
function model(): WorkforceModelDefinition { return { modelId: 'model-test', providerId: 'provider-test', modelName: 'Deterministic Model', capabilities: ['SOFTWARE_ENGINEERING'], contextLimit: 10000, qualityTier: 'STANDARD', expectedLatencyMs: 10, costClass: 'FREE', availability: 'AVAILABLE', routingEligible: true, provenance: 'test' }; }
function makeTask(organizationId: string, taskId: string): WorkforceTaskRequirement { return { taskId, organizationId, requiredCapabilities: ['SOFTWARE_ENGINEERING'], risk: 'LOW', requiredContext: scope, freeFirst: true }; }
function makeProposal(organizationId: string, task: WorkforceTaskRequirement) { const registry = new WorkforceRegistry(); registry.registerCapability(capability()); registry.registerRole(role(organizationId)); registry.registerProvider(provider()); registry.registerModel(model()); const policy = { organizationId, freeFirst: true, requireFreeOption: true, requireProjectContext: true, maxLatencyMs: 1000 }; const selection = buildWorkforceSelectionProposal(organizationId, 'corr-1', [task], [role(organizationId)], registry, policy); return buildDelegationProposal(organizationId, organizationId, 'corr-1', selection, task, role(organizationId), capability().contract); }
async function cleanup(organizationId: string) { await db.auditEvent.deleteMany({ where: { organizationId } }); await db.approval.deleteMany({ where: { organizationId } }); await db.checkpoint.deleteMany({ where: { organizationId } }); await db.job.deleteMany({ where: { organizationId } }); await db.workflow.deleteMany({ where: { organizationId } }); await db.taskDependency.deleteMany({ where: { task: { objective: { organizationId } } } }); await db.task.deleteMany({ where: { objective: { organizationId } } }); await db.objective.deleteMany({ where: { organizationId } }); await db.workerCredential.deleteMany({ where: { worker: { organizationId } } }); await db.worker.deleteMany({ where: { organizationId } }); await db.organization.delete({ where: { id: organizationId } }); }
test.after(async () => { await db.$disconnect(); });

test('Slice 8 delegation reaches the existing durable Phase 1 execution path and workflow completion', async (t) => {
  ...
  try {
    const persisted = await new WorkerRepository(db).create({ organizationId: organization.id, name: 'delegation-worker', type: 'test', capabilities: ['SOFTWARE_ENGINEERING', 'internal.calculate'], credential: 'v208-delegation-worker-credential-123456' }, audit('worker_created', organization.id));
    ...
    const binding: ControlPlaneWorkerBinding = { workerId: persisted.worker.id, organizationId: organization.id, taskId: task.id, delegationId: proposal.proposalId, roleId: 'engineer', capabilities: ['SOFTWARE_ENGINEERING'], context: scope, providerId: 'provider-test', modelId: 'model-test', risk: 'LOW', workflowId: workflow.id };
    ...
    const result = await adapter.executeDelegation(proposal, binding, 'v208-delegation-worker-credential-123456');
    ...
    assert.equal(job?.workerIdentityId, persisted.worker.id);
  ...
  } finally { await cleanup(organization.id); }
});
...
test('duplicate Slice 8 delegation reuses durable Job idempotency', async (t) => {
  ...
  try {
    const persisted = await new WorkerRepository(db).create({ organizationId: organization.id, name: 'delegation-worker', type: 'test', capabilities: ['SOFTWARE_ENGINEERING', 'internal.calculate'], credential: 'v208-duplicate-worker-credential-123456' }, audit('worker_created', organization.id));
